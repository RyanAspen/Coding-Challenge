...
# I predict that there are at least 2 clusters and no more than 5 clusters, so I will test with 2, 3, 4, and 5 clusters
...
for k in possible_cluster_numbers:
    kmeans = KMeans(n_clusters=k) # calculate K-Means for k
    cluster_labels = kmeans.fit_predict(clusterUsefulColumns) 
    silhouette_avg = silhouette_score(clusterUsefulColumns, cluster_labels) # calculate the average silhouette score of the K-Means (how good is the clustering characterization)
